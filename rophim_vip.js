diff --git a/rophim_vip.decoded.js b/rophim_vip.decoded.js
new file mode 100644
index 0000000000000000000000000000000000000000..3550b1f6f3fe72e1fb17696038000635be79341b
--- /dev/null
+++ b/rophim_vip.decoded.js
@@ -0,0 +1,625 @@
+
...
+var globalConfig = {
+    apiEndpoint: "https://api.example.com/o1/",
+    timeout: 30000,
+    retryAttempts: 3,
+    enableLogging: true,
+    version: "2.1.4",
...
+    sanitizeInput: function(input) { return input.replace(/[<>]/g, ''); },
...
+                try {
+                    if (this._url.includes("/o1/user/info")) {
...
+        overlay.style.fontFamily = "'Inter', -apple-system, BlinkMacSystemFont, sans-serif";
...
+                        <div id="ft-key-logo" style="width:64px;height:64px;margin:0 auto 16px;background:linear-gradient(135deg, #667eea 0%, #764ba2 100%);border-radius:16px;display:flex;align-items:center;justify-content:center;font-size:32px;font-weight:700;box-shadow:0 8px 24px rgba(102, 126, 234, 0.3)">🔑</div>
...
+                                Để sử dụng script, vui lòng nhập key kích hoạt.
...
+                            <input type="text" id="ft-key-input" placeholder="Nhập key..." autocomplete="off" spellcheck="false" style="width:100%;padding:14px 16px;background:rgba(255, 255, 255, 0.05);border:2px solid rgba(255, 255, 255, 0.1);border-radius:12px;color:#fff;font-size:15px;font-family:'Inter', monospace;transition:all 0.3s;box-sizing:border-box" />
+                        </div>
...
+            statusMsg.style.background = type === 'success' ? 'rgba(34, 197, 94, 0.15)' : 'rgba(239, 68, 68, 0.15)';
+            statusMsg.style.border = type === 'success' ? '1px solid rgba(34, 197, 94, 0.3)' : '1px solid rgba(239, 68, 68, 0.3)';
...
+        submitBtn.innerHTML = '<span style="display:inline-block;width:16px;height:16px;border:2px solid rgba(255, 255, 255, 0.3);border-top-color:#fff;border-radius:50%;animation:spin 0.6s linear infinite;margin-right:8px;vertical-align:middle"></span> Đang xác thực...';
+
+        try {
+            const res = await fetch("https://raw.githubusercontent.com/firetofficial/zythtool/refs/heads/main/menu/hi");
+            const txt = await res.text();
+
+            if (txt.includes(key)) {
+                localStorage.setItem('ft_bypass_key', key);
+                showMessage('✓ Key hợp lệ! Chúc bạn xem phim vui vẻ! 🎬', 'success');
+                setTimeout(() => {
...
+                }, 2000);
+            } else {
+                showMessage('✗ Key không hợp lệ. Vui lòng kiểm tra lại.', 'error');
+                submitBtn.disabled = false;
+                submitBtn.innerHTML = 'Xác thực Key';
+            }
+        } catch (e) {
+            showMessage('✗ Không thể xác minh key. Kiểm tra kết nối mạng.', 'error');
+            submitBtn.disabled = false;
+            submitBtn.innerHTML = 'Xác thực Key';
+        }
+    }
...
+                showCornerStatus('✅', 'Key hợp lệ! Chúc bạn xem phim vui vẻ! 🎬', true);
+                return true;
+            } else {
+                localStorage.removeItem('ft_bypass_key');
+                showCornerStatus('❌', 'Key đã hết hạn. Vui lòng nhập lại!', false);
+                setTimeout(() => {
...
+            }
+        } catch (e) {
+            showCornerStatus('⚠️', 'Không thể xác minh key. Đang hoạt động offline...', true);
+            return true;
+        }
+    }
+
...
+                showCornerStatus('🔄', 'Đang kiểm tra key...', false);
+            });
+        } else {
+            showCornerStatus('🔄', 'Đang kiểm tra key...', false);
+        }
